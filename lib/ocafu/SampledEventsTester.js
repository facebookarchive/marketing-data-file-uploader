...
var testSampledEvents = exports.testSampledEvents = function testSampledEvents(configs, callback) {
  ...
  rstream.on('error', function (err) {
    callback(err);
  }).pipe(split()).pipe(batchHandler).pipe(through.obj(function (batch, enc, cb) {
    if (batch.length > 0) {
...
var checkSampledEvents = function checkSampledEvents(normalizedEvents, configs, callback) {
  (0, _Logger.getLogger)().info("STEP 1. Sampled events validation test");
  var err = checkInvalidSignalRate(normalizedEvents);
  if (err) {
    callback(err);
  } else {
...
var checkInvalidSignalRate = function checkInvalidSignalRate(normalizedEvents) {
